...
| 13 | Large-library responsiveness | Two main-thread stalls that only show up past ~30k tracks, both measured on a 37,477-track library (2026-08-09). **(a) `rebuild_files()`** calls `all_tracks()` and rebuilds the whole store: 380 ms for the query alone, before ~37k `BoxedAnyObject` allocations, the splice, and GTK re-sorting every row. It fires on every search debounce tick, rescan completion, ID3 save and folder add. **(b) `rebuild_playlist()`** inserts all 37,475 rows into the TreeView's `ListStore` on every playlist change. Neither touches disk — this is row count, so the fix is virtualisation / paging / off-thread query, not I/O. Also fold in the deferred DB-only status glyph (compare stored `file_mtime` vs `last_scanned` like `replaygain::needs_analysis` already does, dropping the per-row `stat`+`open` and the session cache added in `2c27fd3`) | Added 2026-08-09 after the step-5 test pass surfaced app-wide lag. Deliberately last: it is a scaling problem, not a parity gap, and the cheap wins were already taken — the 13,000× one (`Track::from_path` → `Track::from(&LibTrack)` on playlist add, 1,048 s → 0.08 s) landed in `152ced6`. What remains needs a design decision on how the ML presents a large library, so it wants its own spec rather than a patch |
