...
- [x] **Step 1: ImageCaptureCore detector.** `UnsupportedDeviceWatcher` wraps `ICDeviceBrowser` (delegate `didAdd`/`didRemove`, mask = camera | local). Each device becomes a synthetic `Device` with `backend = .unsupported`, `fsVisible = false`, `readOnly = true`, zero capacity, `backendId = uuidString`, `mountPath = ""`, `label = name`; `fsType` carries "ios"/"ptp" (Apple detected via USB vendor 0x05AC or name) to drive the banner text.
- [x] **Step 2: Merge into the device list.** Publishes into `model.unsupportedDevices`; `model.allDevices = devices + unsupportedDevices` feeds the sidebar count/rows, overview grid, and detail lookup. Sidebar rows and overview cards branch on `.unsupported` (iPhone/camera icon, no capacity bar). Watcher started on ML-window appear, stopped on disappear.
- [x] **Step 3: Banner in the detail view.** `DeviceDetailView` shows `unsupportedBanner` (Apple: "Music sync isn't supported for iPhone or iPad…" / PTP: "Photo-transfer device…") for `.unsupported`, with a cleaner header subtitle. `loadDeviceTracks`/`loadDevicePlaylists` already no-op when `!fsVisible`, so no `browse`/`syncPlan` runs; Add/Sync/Scan are disabled and Eject is hidden (not ejectable).
- [x] **Step 4: Entitlements.** None needed — the app is unsandboxed (see Task 9 Step 2). ImageCaptureCore autolinks via `import` and enumerates PTP devices without a TCC prompt.
- [ ] **Step 5: Verify + commit.** `xcodebuild` succeeds, zero warnings. Manual iPhone plug-in test is the user's device run. Commit `feat(macos): recognize iOS/PTP devices as non-syncable`.
