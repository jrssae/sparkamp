...
**Interfaces:**
- Produces: `pub fn apply_watch_action(&self, action: &crate::watch::WatchAction, remove_missing: bool) -> Result<()>` on the ML handle. `Upsert(path)`: resolve the owning folder (nearest ancestor of the path among rows in `folders`; if none, `folder_id = NULL`), fast-insert the path row if absent (mirror `rescan_folder_fast`'s single-row insert), then `upsert_track(folder_id, path)`. `Remove(path)`: if `remove_missing` delete the row (`DELETE FROM tracks WHERE path = ?1`), else mark broken (existing mark-broken behavior — reuse whatever `scan.rs` already does for a missing file; grep `deleted_at`/mark-broken and match it).
...
- [ ] **Step 2: Run, verify fail.**
- [ ] **Step 3: Implement.** `compact()` = `self.conn.execute("VACUUM", [])?`. Thread the `remove_missing` bool into the existing missing-file handling in `scan_folder` (the `DELETE FROM tracks WHERE id = ?1` at `scan.rs:337/450` vs mark-broken). Callers pass the config flag; VACUUM runs after a full `scan_all_folders` completes only when `compact_on_rescan` is on (caller-driven, so `compact()` stays a plain method).
- [ ] **Step 4: Run tests → PASS**, full suite.
