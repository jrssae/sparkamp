...
- [x] **Task 2.4 — tag override UI** (mac + TUI; **GTK done**). Overrides are per-disc tag sets keyed by freedb id, editable **with or without a match**, overlaid onto the track titles and consumed by rip (P3) / submission (P4). macOS: Identify button (auto-applies a single exact match; sheet picker otherwise) + Edit Tags sheet (disc fields + per-track titles), artist—album shown in the drive header. TUI: `m` identify (background thread + tick-drained channel — the UI never blocks on the network), match overlay (↑↓/Enter/Esc), `e` tag editor overlay (rows = disc fields + titles; Enter edits, Esc saves). **GTK:** Identify button (background gnudb query via `gio::spawn_blocking`; single exact auto-applies, several open a modal picker, none → status pointing at Edit Tags) + Edit Tags dialog (Artist/Album/Year/Genre + per-track title entries, editable with or without a match, preserves extd/extt/revision); tags load from the shared `disc_tags.toml` store on startup, persist on identify/save, overlay onto the track list + "Artist — Album" drive header, and propagate immediately into already-added active-playlist rows (path-keyed, same " / " sampler split as adds).
...
- [x] **Progress + cancel** — mac: per-track progress bar in the drive view + Cancel (stops after the current track), rip loop on a worker queue. TUI: rip overlay (`g`; Space/a select, q quality, d dest, Enter) with progress in the hint bar and `c` to cancel; background thread + tick-drained channel. **GTK done:** a "Rip…" button opens a modal (track multi-select preselected from the highlighted rows else all, destination Entry + Browse defaulting `rip_dest_dir` → first watched folder → ~/Music with a warn-only unwatched-folder notice, quality dropdown V0/V2/320), choices remembered in config; the rip runs on a worker thread with a `ProgressBar` + Cancel in the drive detail (mpsc drained by a `glib::timeout`), then imports via `add_files_to_library` with the same honest status. The cd-info poll is suppressed during a rip (same optical-reader contention as playback). Core Cdda rip path live-verified in dev-box (3 MB tagged MP3 from the real disc).
...
- [x] **UI (deviating per user request):** the mac drive view's redundant Scan button is **replaced** by "Submit to gnudb", shown only when the disc is unmatched (always) or the tags differ from the official match (`discSubmittable`). TUI: `u` in the Discs tab. **GTK done:** a "Submit to gnudb" button in the drive actions row (same visibility rule — `disc_submittable` mirrors the mac field comparison, refreshed by `populate_disc_detail`), local `validate_for_submit` before any network, email captured via the shared `prompt_gnudb_email` modal when unset, then a modal category picker (fixed 11-set, prefilled by `suggest_category`, test-mode note shown when on) and a background POST via `gio::spawn_blocking`; results land in the disc status line, test-mode results labeled "(test mode — not published)".
...
- [x] **Drive disconnect** (core/mac/TUI 2026-07-10; GTK 2026-07-12) — the drive being viewed vanishing mid-session now invalidates its loaded-disc session and surfaces a banner instead of silently dropping out. **mac:** `discDisconnectNotice` → dismissible banner on the Disc Drives overview, `discTracks` cleared, nav falls back. **TUI:** `refresh_ml_drives` diffs the prior selected drive id; if gone → "Drive disconnected — reconnect and reload (r)" status + entries rebuilt for the new selection. **GTK:** the poll's unplug fallback now clears the disc entries and shows a dismissible "Drive disconnected — reconnect it to continue" banner on the discs overview (hidden again when any drive detail opens). In-flight subprocess ops still die with the device (unchanged).
...
- [x] **Commit** `feat(disc): graceful drive/disc/burn error handling` (core/mac/TUI in `cb4dbb8`; GTK banner + first Linux compile of the split follow).
