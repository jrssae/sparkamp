...
- [ ] **Enumerate every optical drive.** Linux: discover all optical nodes (`/dev/sr*`, via `cd-info`/libcdio or sysfs) and, for each, parse `OpticalDrive { media: MediaInfo, toc }`. macOS: Swift IOKit enumerates optical drives and, for a loaded audio CD, passes the mounted track file list + TOC (from the `.TOC.plist`) into core via FFI. Poll on the existing device timer.
- [ ] **"Disc Drives" sidebar group.** Show a dedicated Media Library section that lists **one row per physical drive present** — mirroring how the external-devices section lists every device (never collapse to a single drive, no picker). Each row shows the drive label + loaded-media state, e.g. "PIONEER DVD-RAM — Audio CD (12 tracks)", "— Blank CD-R", "— Data disc", "— No disc". Selecting a drive opens its detail view (track list for an audio disc; burn target for blank/data media).
- [ ] **Add-to-playlist URIs.** Provide, per track, a playable URI: Linux `cdda://N` (playbin plays it directly, device set via the `cdda` URI/property, scoped to the selected drive's node); macOS `file://…/N Audio Track.aiff`. Playlist entries carry title/artist/duration when known (from a prior gnudb match, else "Track N").
- [ ] **UI: add individual tracks or whole disc** to the active playlist (parity: Winamp "add track / add all").
- [ ] **Tests:** `DiscToc` → discid vectors (see Phase 2 Task) and URI construction. Manual: real DVD-RAM drive plays an audio CD track and the full disc.
