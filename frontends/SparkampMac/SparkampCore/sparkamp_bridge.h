...
/* Post-burn disc verification where the tool supports it (default true). */
bool    sparkamp_get_burn_verify(SparkampCtx *ctx);
void    sparkamp_set_burn_verify(SparkampCtx *ctx, bool value);
/* Auto-open the Media Library to a drive that just received an audio CD
 * (default true). Effective only while running; OS default-handler
 * registration is a separate manual step. */
bool    sparkamp_get_auto_show_inserted_cd(SparkampCtx *ctx);
void    sparkamp_set_auto_show_inserted_cd(SparkampCtx *ctx, bool value);
...
/** Build the lyrics view for `path` and return it as a heap JSON string:
    ...
    the file has no lyrics; the window still opens and shows "No lyrics
    available"). Free the result with sparkamp_free_string. A NULL path
    returns NULL. */
char           *sparkamp_lyrics_view(const char *path, const char *artist,
...
// ---------------------------------------------------------------------------
// Optical discs (CD/DVD drives)
// ---------------------------------------------------------------------------
//
// JSON-over-FFI like the device API; free every returned string with
// sparkamp_free_string. Detection runs drutil/plutil subprocesses — call from
// a background queue and throttle polling. See src/ffi/disc.rs for the JSON
...
/** Playlist-ready entries for the drive's audio tracks as a JSON
    [DiscTrackEntry] {number,path,title,duration_secs}; empty array when no
...
int sparkamp_disc_erase_decision(SparkampCtx *ctx, const char *drive_json);
