...
/// EXACTLY — keep in sync. Produces `N tracks · MM:SS total · K selected ·
/// MM:SS`; the selected clause is present only when `selected` is non-nil.
/// Durations roll over to `H:MM:SS` at/above one hour.
///
...
func playlistStatusLine(count: Int, totalSecs: Int, selected: (count: Int, secs: Int)?) -> String {
    ...
    if let sel = selected { line += " · \(sel.count) selected · \(hms(sel.secs))" }
    return line
}
