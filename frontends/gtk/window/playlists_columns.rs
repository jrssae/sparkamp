    ...
    }
    {
        ...
        for c in ALL_COLUMNS.iter() {
            ...
            factory.connect_bind(move |_, obj| {
                ...
                // HIG's list guidance asks for. Set on every column's cell —
                // not just one — because which column is leftmost depends on
                // the user's own column picker. `spoken_row_summary` lives in
                // `files.rs`: the Files, Playlist-editor and Device views all
                // bind the same `LibTrack` fields, so one function serves all
                // three rather than three copies of the same `match`.
