...
fn open_media_library_window(
    ...
    // These cells belong to the Devices page, not to the sidebar. They were
    // declared above only because the file grew widgets-first; nothing
    // between their old position and here used them. Parking them below
    // leaves the sidebar contiguous so it could be extracted, and they
    // travel to the Devices page in plan step 6 the way the Discs page's
    // cells travelled to `disc_page.rs` in step 5.
