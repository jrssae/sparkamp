...
fn open_media_library_window(
    ...
    *rebuild_track_list_holder.borrow_mut() = Some(rebuild_track_list.clone());
    ...
    // ── Page: Devices ────────────────────────────────────────────────────
    // Everything from here to the window tail is the Devices page, reunited
    // (plan step 6, the hoist): its widgets used to sit at the very top of
    // this function, ~3,200 lines above the poll and the selection handler
    // that drive them, because the file grew widgets-first and wiring-last.
    // Nothing outside referred to any of it except three names, and those now
    // reach it through holders, so the block moved down here in one piece
    // rather than the wiring moving up — the same cheaper direction step 5
    // took, and for the same reason: moving the wiring up would strand it
    // above `stack`, `ctx` and the editor it is declared after.
    //
    // The next commit lifts this block out to `devices_page.rs`.
