...
fn open_media_library_window(
    ...
    *rebuild_track_list_holder.borrow_mut() = Some(rebuild_track_list.clone());
    ...
    // ── "Disc Drives" content page — moved down (plan step 5a) ──────────
    // The disc UI was split in two by the file's widgets-first order: this
    // block sat ~4,600 lines above the wiring that drives it. The two halves
    // are reunited here so step 5b can lift them together.
    //
    // The widgets moved down rather than the wiring moving up, which is the
    // opposite of what the plan sketched. Hoisting the wiring would have
    // landed it above `stack`, the Files column holders, the playlist
    // editor's `track_list` and four device-wiring closures it captures.
    // Moving the widgets down crosses only one reference — the stack hookup,
    // which comes along — and adds scope rather than removing it.
