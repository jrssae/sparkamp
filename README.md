...
The Linux audio player ecosystem is genuinely strong — but the specific combination of features that made Winamp's workflow so satisfying has been hard for me to find in one single app. SparkAmp is a personal attempt to build exactly that: an audio player that gives me thing things from Winamp that I miss most since leaving Windows. If those are the things you've been missing too, this might be for you.

> **This project is entirely vibe coded.** I am neither a programmer or a designer — every line of code was written by Claude (Anthropic's AI assistant). Human coders and designers are genuinely welcome and actively encouraged to contribute. If you see something that can be done better, please open a PR. I have no idea what I'm doing and some experience is very beneficial. The goal is a great piece of software, not a monument to any particular development process.
...
- **Jump-to-track** — press `j` to open a live search window, navigate with arrow keys or keyboard, press Enter to jump and play
- **Background duration probing** — file lengths appear immediately on load without blocking the UI, with a fallback path for CBR MP3s that lack a Xing header
- **Missing file detection** — files that disappear from disk are marked with a warning indicator rather than silently failing
- **Duration cache** — probed durations are persisted to disk so they appear instantly on next launch
- **Mini visualizer** — bars or oscilloscope mode
- **CLI mode** — a full terminal UI for headless or keyboard-only use (`sparkamp --tui`)
- **Playlist persistence** — saves and restores the playlist and player position between sessions
- **Skin system** — light and dark CSS skins, switchable at runtime (foundation for future custom skins)
...
- **Settings panel** — ability to start configuring behaviors in a clear UI
- **Expanded format support** — broader codec coverage beyond the current MP3 / FLAC / OGG / WAV / M4A set
- **Media library** — artist/album browsing, full ID3 tag display and editing
- **Custom visualizers** — user-defined visualizer plugins or configurations
- **Skin format and migration tool** — a new, skin format with a migration path for classic Winamp `.wsz` skins
- **Equalizer** — per-band EQ with presets
- **OSX Support** - would be nice to have on my laptop
